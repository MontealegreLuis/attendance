...
class AttendanceGeneratorCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $bootcamps = $this->currentBootcamps($today);
        $absentStudents = $this->absentStudents($bootcamps);
        $students = $this->selectRandomStudents($absentStudents);
     ...
     * @return array
     */
    protected function absentStudents(array $bootcamps)
    {
        $builder = $this->connection->createQueryBuilder();
        $builder
            ->addSelect('b.bootcamp_id')
     ...
     * @return array
     */
    protected function selectRandomStudents(array $absentStudents)
    {
        $studentsIds = [];
        foreach ($absentStudents as $bootcampId => $students) {
            shuffle($students);
            $studentsIds[$bootcampId] = array_slice(
                $students,
                0,
                mt_rand(1, 5)
            );
    ...
    {
        foreach ($randomStudents as $bootcampId => $students) {
